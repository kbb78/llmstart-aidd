...
`Assistant` собирает запрос: первым идёт `system` из `SYSTEM_PROMPT`, затем история чата. В вызов LLM передаётся `max_tokens` из `LLM_MAX_TOKENS` (по умолчанию 500). Другие параметры сэмплинга в конфиг не выносим.
...
| `LLM_MAX_TOKENS` | нет | лимит токенов ответа модели, по умолчанию `500` |

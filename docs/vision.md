...
| Фото | тот же `base_url`, vision-модель, изображение → текст |
| Аудио | тот же `base_url`, STT (`/audio/transcriptions`), `input_audio`, формат как в Telegram (`ogg`) |
| Конфиг | переменные окружения, файл `.env` |
| Логи | стандартный модуль `logging` |
| История диалога | память процесса, без БД |

Переключение провайдера — переменная `LLM_PROVIDER` в `.env` (`openrouter` или `ollama`). URL по умолчанию выбирает `Config`. Если задан `LLM_BASE_URL` — его используют текст, фото и аудио. Ключ и имена моделей — отдельные переменные. Код вызова текстового LLM один: клиент `openai`.

Модели и промпты фото и аудио — отдельные переменные; URL у них тот же, что у текста.
...
6. **Без абстракций «на вырост».** Текстовый LLM — конфиг одного клиента `openai`. Фото и аудио — отдельные клиенты с одним методом; URL — тот же `Config.base_url`. Другую реализацию подставляют в `__main__.py`. Нет DI-контейнера, ABC и фабрик.
...
| `vision_client.py` | `VisionClient` | изображение → текст; `base_url` из `Config` |
| `audio_client.py` | `AudioClient` | аудио → транскрипт; STT `input_audio`, `base_url` из `Config` |
...
- Клиенты — точки расширения: в `__main__.py` можно подставить другую реализацию с тем же методом. URL у текста, фото и аудио — `Config.base_url`.
- `Config` отдаёт токен бота, провайдера (`LLM_PROVIDER`), URL, ключ, модели и промпты, лимит токенов ответа.
...
`Config` выбирает `base_url` для текста, фото и аудио:
...
**Фото.** `VisionClient` вызывает chat completions с vision-моделью: текст промпта (`VISION_PROMPT`) и изображение (base64). URL — `Config.base_url`. На выходе — текст: что на фото и что с ним делать в занятии. Подпись к фото, если есть, передаётся вместе с изображением: пользователь может попросить объяснить снимок.

**Аудио.** `AudioClient` вызывает `POST {base_url}/audio/transcriptions`: raw base64 и `format` файла (для голосовых Telegram — `ogg`). Модель — `AUDIO_MODEL` (например Whisper). Без конвертации в WAV и без `ffmpeg`. На выходе — текст реплики пользователя.

Ключ — `LLM_API_KEY`. URL тот же, что у текстового LLM: `LLM_BASE_URL` или таблица по `LLM_PROVIDER`.
...
| `VISION_MODEL` | да | vision-модель у провайдера |
| `VISION_PROMPT` | да | промпт разбора фото |
| `AUDIO_MODEL` | да | STT-модель (например Whisper) |
| `AUDIO_PROMPT` | да | зарезервировано; в STT-запрос не передаётся |
| `LLM_API_KEY` | для OpenRouter — да; для Ollama — нет (заглушка `ollama`) | ключ API для текста, фото и аудио |
| `LLM_BASE_URL` | нет | перекрывает URL провайдера для текста, фото и аудио |
...
В git: `.env.example`. Файл `.env` в `.gitignore`. Смена провайдера: `LLM_PROVIDER` (плюс ключ и модель), перезапуск. Смена модели или промпта фото/аудио — те же переменные, код не меняется.

...
`Assistant` собирает запрос: первым идёт `system` из `SYSTEM_PROMPT`, затем история чата. В вызов LLM передаётся `max_tokens` из `LLM_MAX_TOKENS` (по умолчанию 2000). Другие параметры сэмплинга в конфиг не выносим.
